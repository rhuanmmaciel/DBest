...
public class DslController {
    ...
    private static void importTable(String importStatement) {
		...
		String clearedTableName = DslUtils.clearTableName(tableName);

		if (MainController.getTables().containsKey(clearedTableName)) {
			DslErrorListener.addErrors(String.format("Já existe uma tabela com o nome '%s'", clearedTableName));
		} else if (!FileUtils.copyDatFilesWithHead(path, tableName, Path.of(""))) {
			DslErrorListener.addErrors(String.format("Arquivo '%s.head' ou '%s.dat' não encontrado", clearedTableName, clearedTableName));
		} else {
			MainController.saveTable(new FyiTableCell(
				null, clearedTableName, "fyi",
				Table.loadFromHeader(String.format("%s.head", clearedTableName))
			));
		}
    }
